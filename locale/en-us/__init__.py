...
class Zod(MycroftSkill):
    def __init__(self):
        ...
        # self.device_id = self.check_device_id() if self.check_device_id() else self.create_new_id()
    ...
    #         content = {}
    #         entry_name = "Entries"
    #         content[entry_name] = [0]
    #         date = (str(datetime.datetime.now().strftime('%d/%m/%y')))
    #         content[0] = {"register": True, "device_id": new_id, "date": date}
    #         with open(device_cd,'w+') as outfile: # Overwrite content from device log
    #             js.dump(content,outfile, indent= 2)
    #         return new_id
    #     except:
    #         print("[Warning] Unable to create ID")
    #         LOG.info(f"[Warning] Unable to create ID")
    #         return 0
